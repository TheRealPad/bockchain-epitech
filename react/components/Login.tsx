...
const Login: React.FC<LoginProps> = ({ login }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/wallet/${walletAddress}`,
        {
          method: "GET",
        },
      );
  ...
  return (
    <div style={styles.container} className="relative">
      <div className="absolute inset-0 bg-[url(/background-login.jpeg)] blur-sm"></div>
      <div style={styles.formContainer} className="z-10">
        ...
        <div style={styles.createWalletSection}>
          ...
          {generatedWallet && (
            <div style={styles.generatedWalletInfo}>
              <p style={{ color: "black" }}>
                <strong>Wallet Address:</strong> {generatedWallet.address}
              </p>
              <p style={{ color: "black" }}>
                <strong>Secret:</strong> {generatedWallet.secret}
              </p>
              <p style={{ color: "black" }}>
                <strong>Public Key:</strong> {generatedWallet.public_key}
              </p>
              <p style={{ color: "black" }}>
                <strong>Private Key:</strong> {generatedWallet.private_key}
              </p>
              <p style={styles.warning}>
                ⚠️ Save your secret key securely. It will not be shown again!
              </p>
            </div>
          )}
        </div>
